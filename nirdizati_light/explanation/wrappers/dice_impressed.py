...
def dice_impressed(CONF, predictive_model, cf_df, encoder, query_instance, query_case_id, method, support,optimization,
    ...
    """
    logger.debug("dice_impressed()")
    logger.debug(f"Activity original position received in dice_impressed(): {activity_origin_position}")
    ...
    try:
        ...
        logger.debug(f'Unable to save {model_path}. Error: {e}')
    ...
    try:
        ...
        logger.debug(f'Unable to save {model_path + dataset} + _ + {CONF["prefix_length"]} + .decl. Error: {e}')
    ...
    try:
        ...
        logger.debug(f'Error evaluating cf_list. Error: {e}')
    ...
    try:
        ...
        # print(long_data_sorted[timestamp_col_name])
    except Exception as e:
        print(f'Error converting timestamp column to datetime. Error: {e}')
    ...
    try:
        ...
        print(f'Cannot convert to event log, saving dataframe. Error: {e}')
    """
    
    ### Add the source (inquiry) Case ID
    event_log["Query_CaseID"] =  query_case_id
